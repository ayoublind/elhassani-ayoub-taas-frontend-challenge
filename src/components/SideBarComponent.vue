        ...
        <div class="mb-4 px-4">
            <div style="height: 700px;" class=" overflow-y-auto">
                ...
                <ul v-else class="space-y-4 text-left text-gray-500 dark:text-gray-400">
                    <li class="cursor-pointer flex items-center space-x-3 " v-on:click="handleSelectRepo(repo)"
                        v-for="(repo, index) in repos" :key="index">
                        <svg v-if="selectedRepo && repo.full_name == selectedRepo.full_name"
                            class="flex-shrink-0 w-3.5 h-3.5 text-green-500 dark:text-green-400" aria-hidden="true"
...
export default {
    ...
    methods: {
        handleSelectRepo(repo) {
            ...
            this.$emit('selectRepository', repo)
        },
